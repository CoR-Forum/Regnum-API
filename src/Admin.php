...
class Admin {
    ...
    public function getAllUsers() {
        $this->checkAdmin();
        $stmt = $this->pdo->prepare('SELECT id, username, nickname, email, is_active, is_admin, shoutbox_banned, created_at, updated_at, banned, last_login, last_activity FROM users WHERE deleted = 0');
        $stmt->execute();
